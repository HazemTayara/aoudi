...
Route::middleware('guest')->group(function () {
    ...
    // Step 2: Verify OTP (using token, not email in URL)
    Route::get('verify-otp/{token}', [ResetPasswordOtpController::class, 'showVerifyForm'])->name('password.verify.form');
    Route::post('verify-otp', [ResetPasswordOtpController::class, 'verifyOtp'])->name('password.verify');

    // Step 3: Reset password (using token, not email)
    Route::get('reset-password/{reset_token}', [ResetPasswordOtpController::class, 'showResetForm'])->name('password.reset.form');
    Route::post('reset-password', [ResetPasswordOtpController::class, 'reset'])->name('password.update');
});
